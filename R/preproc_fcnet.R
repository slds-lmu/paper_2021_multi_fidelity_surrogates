preproc_data_fcnet = function(config, seed = 123L, n_max = 5*10^6, frac = .1) {
  ...
  trafos = c(
    map(train[, c("n_params", "runtime"), with = FALSE], scale_base_0_1, base = 10, p = 0),
    map(train[, c("valid_loss"), with = FALSE], scale_neg_exp),
    map(train[, c("batch_size", "n_units_1", "n_units_2"), with = FALSE], scale_base_0_1, base = 1, p = 0),
    map(train[, c("init_lr"), with = FALSE], scale_base, base = 10)
  )
