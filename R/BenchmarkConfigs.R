  ...
  public = list(
    initialize = function(id = 'RBv2_SVM', workdir) {
      super$initialize(
        ...
        onnx_model_file = "model.onnx",
        budget_param = "epoch",
        target_variables = c("perf.logloss.mean", "perf.mmce.mean", "timetrain.mean", "timetest.mean"),
        codomain = ps(
          perf.logloss.mean = p_dbl(lower = 0, upper = 1, tags = "minimize"),
          perf.mmce.mean = p_dbl(lower = 0, upper = 1, tags = "minimize"),
          timetrain.mean = p_dbl(lower = 0, upper = 1, tags = "minimize"),
          timetest.mean = p_dbl(lower = 0, upper = 1, tags = "minimize")
        ),
        packages = NULL
      )
    }
