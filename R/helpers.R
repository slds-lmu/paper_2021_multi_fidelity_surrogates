...
scale_sigmoid = function(x, p = 0.03) {
  ...
  cat("Sigmoid-scaling [", min(x), ";", max(x), "] to [", (min(x) - rt_min) / rt_range + p, ";", (max(x) - rt_min) / rt_range + p,"]\n")
  list(
...
scale_base = function(x, base = 10) {
  ...
  cat("Log-scaling [", min(x), ";", max(x), "] to [", log(min(x), base)/div, ";", log(max(x), base)/div ,"]\n")
  list(
